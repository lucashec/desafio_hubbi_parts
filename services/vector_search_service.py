...
class VectorSearchService:
    """Service for semantic search using embeddings (RAG)."""
    ...
    def search_parts_by_query(self, query: str, top_k: int = 10, min_score: float = 0.3) -> List[Dict]:
        """
        Search for parts using semantic similarity with RAG.
        ...
        # Search in database
        results = []
        parts = Part.objects.filter(quantity__gt=0).all()
        
        for part in parts:
            if not part.embedding:
                # Generate embedding if not exists
                self._generate_and_save_embedding(part)
                if not part.embedding:
                    continue
            
            # Convert bytes to array
            part_embedding = self.embedding_service.bytes_to_array(part.embedding)
            if part_embedding is None:
                continue
            
            # Calculate similarity
            similarity = self.embedding_service.compute_similarity(query_embedding, part_embedding)
    ...
    def _generate_and_save_embedding(self, part: Part) -> bool:
        """Generate and save embedding for a part."""
        try:
            # Combine name and description for embedding
            text = f"{part.name} {part.description}".strip()
            
            embedding_bytes = self.embedding_service.generate_embedding(text)
            if embedding_bytes:
                from django.utils import timezone
                part.embedding = embedding_bytes
